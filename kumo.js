var app = new Vue({
  ...
  methods: {
    apply: function(event) {
      ...
      Promise.all(config.sources.reduce(function(promises, item) {
        return promises.concat(setSourceConfig(this.address, item))
      }.bind(this), [])).then(function() {
        console.log("source config success!");
      }.bind(this));
      Promise.all(config.destinations.reduce(function(promises, item) {
        return promises.concat(setDestinationConfig(this.address, item))
      }.bind(this), [])).then(function() {
        console.log("destination config success!");
      }.bind(this));
      console.log("apply to", this.address);
    },
  }
});
